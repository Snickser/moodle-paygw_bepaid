...
/**
 * Privacy Subsystem implementation for paygw_bepaid.
 *
...
namespace paygw_bepaid\privacy;
...
class provider implements \core_privacy\local\request\data_provider, paygw_provider, \core_privacy\local\metadata\provider {
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $collection->add_external_location_link(
            'bepaid.plus',
            [
                'shopid'   => 'privacy:metadata:paygw_bepaid:shopid',
                'apikey'   => 'privacy:metadata:paygw_bepaid:apikey',
                'email'    => 'privacy:metadata:paygw_bepaid:email',
            ],
            'privacy:metadata:paygw_bepaid:bepaid_plus'
        );

        // The paygw_bepaid has a database table that contains user data.
        $collection->add_database_table(
            'paygw_bepaid',
            [
                'invoiceid'  => 'privacy:metadata:paygw_bepaid:invoiceid',
                'courseid'   => 'privacy:metadata:paygw_bepaid:courseid',
                'groupnames' => 'privacy:metadata:paygw_bepaid:groupnames',
                'success'    => 'privacy:metadata:paygw_bepaid:success',
            ],
            'privacy:metadata:paygw_bepaid:paygw_bepaid'
        );
        return $collection;
    }
    ...
    public static function export_payment_data(\context $context, array $subcontext, \stdClass $payment) {
        ...
        $subcontext[] = get_string('gatewayname', 'paygw_bepaid');
        $record = $DB->get_record('paygw_bepaid', ['paymentid' => $payment->id]);
    ...
    public static function delete_data_for_payment_sql(string $paymentsql, array $paymentparams) {
        ...
        $DB->delete_records_select('paygw_bepaid', "paymentid IN ({$paymentsql})", $paymentparams);
    }
}
