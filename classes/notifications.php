...
/** Notifications for paygw_bepaid.
 *
...
class notifications {
    /**
     * Function that handle the notifications about transactions using bepaid payment gateway
    ...
    public static function notify($userid, $fee, $currency, $orderid, $type = '', $nextpay = '') {
        ...
        $message->component = 'paygw_bepaid';
        $message->name      = 'payment_receipt'; // The notification name from message.php.
        $message->userfrom  = \core_user::get_noreply_user(); // If the message is 'from' a specific user you can set them here.
        $message->userto    = $user;
        $message->subject   = get_string('messagesubject', 'paygw_bepaid', $type);
        switch ($type) {
            case 'Success completed':
                $messagebody = get_string('message_success_completed', 'paygw_bepaid', $a);
                break;
            case 'Success recurrent':
                $messagebody = get_string('message_success_recurrent', 'paygw_bepaid', $a);
                break;
            case 'Recurrent created':
                $messagebody = get_string('message_recurrent_created', 'paygw_bepaid', $a);
                break;
            case 'Recurrent error':
                $messagebody = get_string('message_recurrent_error', 'paygw_bepaid', $a);
                break;
            case 'Recurrent notify':
                $messagebody = get_string('message_recurrent_notify', 'paygw_bepaid', $a);
                break;
            case 'Invoice created':
                $messagebody = get_string('message_invoice_created', 'paygw_bepaid', $a);
                break;
            default:
                $messagebody = 'null';
        }
